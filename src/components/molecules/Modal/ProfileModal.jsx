...
const ProfileModal = () => {
  ...
  const { token, account, isUpdate, showModal } = useSelector((state) => state.globalReducer);
  const { user } = account;
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <form className="p-2 w-full" onSubmit={formik.handleSubmit}>
          ...
          <div className="w-full h-28 overflow-x-auto">
            {[...sosialMedia]?.reverse()?.map((item, index) => (
              <div key={index} className="w-full flex gap-2 flex-row  my-2">
                ...
                <Button tydark:border-primary-bluepe="button" onClick={(event) => handleDeleteSocialMedia(event, index)} className="text-red-600 ">
